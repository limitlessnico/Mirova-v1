...
def procesar():
    ...
    for v in VOLCANES:
        ...
        if not df_v.empty:
            ...
            # --- ETIQUETA MÁXIMO (2 Decimales corregidos) ---
            ...
            # --- CORRECCIÓN DE EJES Y GRILLA ---
            # Forzamos a que el eje X siempre muestre los 30 días
            fig.update_xaxes(
                range=[hace_30_dias, ahora],
                showgrid=True, 
                gridcolor='#333', 
                dtick=86400000.0 * 5, # Etiqueta cada 5 días para evitar solapamiento
                tickformat="%d %b", # El formato se ajusta en layout para español
                tickangle=-45
            )
            
            # Grilla horizontal clara para todos los días
            fig.update_yaxes(
                showgrid=True, 
                gridcolor='#333', 
                title="Potencia Radiada (MW)", 
                range=[0, max(1.5, df_v['VRP_MW'].max() * 1.4)]
            )
            ...
            # Estado Nominal si no hay datos en 30 días
            with open(ruta_v, "w") as f: 
                f.write("<body style='background:#161b22; display:flex; align-items:center; justify-content:center; height:100vh; margin:0;'><div style='color:#8b949e; font-family:sans-serif; font-size:16px; border: 1px dashed #30363d; padding: 20px; border-radius:8px;'>SIN ANOMALÍA TÉRMICA (30 DÍAS)</div></body>")
