...
def limpiar_todo_para_test():
    """
    MODO HARD RESET: 
    Borra TODO para probar que el scraper es capaz de reconstruir 
    la base de datos desde la tabla online.
    """
    # 1. Borrar carpeta vieja (legacy)
    if os.path.exists(CARPETA_OBSOLETA):
        try: shutil.rmtree(CARPETA_OBSOLETA)
        except: pass
        
    # 2. BORRADO NUCLEAR DE LA CARPETA PRINCIPAL
    # Esto elimina CSVs y FOTOS actuales.
    if os.path.exists(CARPETA_PRINCIPAL):
        print(f"💥 BORRANDO HISTORIAL COMPLETO ({CARPETA_PRINCIPAL}) PARA PRUEBA LIMPIA...")
        try: shutil.rmtree(CARPETA_PRINCIPAL)
        except Exception as e: print(f"Error borrando: {e}")
...
def procesar():
    ...
    print(f"🚀 Iniciando V19.1 (REINICIO TOTAL): {ahora_cl}")
    ...
    try:
        ...
        if tabla:
            ...
            # Como borramos todo, db_keys está vacío
            db_keys = set() 
            ...
            for fila in filas:
                ...
                try:
                    ...
                    if clave in ids_procesados: continue
                    ...
                    print(f"🔥 RECUPERANDO: {nombre_limpio} ({sensor_str}) | {fecha_fmt} | VRP:{vrp_val}")
            ...
            if registros_nuevos:
                ...
                # Invertimos el orden para que lo más nuevo quede abajo (opcional, pero ordenado)
                df_new = df_new.sort_values(by="timestamp")
                
                df_new.to_csv(DB_FILE, index=False)
                print(f"💾 Base de datos reconstruida con {len(registros_nuevos)} registros.")
                ...
                print("💤 No se encontraron datos para tus volcanes en la tabla actual.")
