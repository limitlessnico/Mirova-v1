...
def procesar():
    ...
    for v in VOLCANES:
        ...
        fig.update_xaxes(type="date", range=[hace_30_dias, ahora], tickvals=ticks_x, ticktext=labels_x, 
                         showgrid=True, gridcolor='rgba(255,255,255,0.08)', tickangle=-45, fixedrange=True, tickfont=dict(size=9))
        
        fig.update_yaxes(range=[0, max(1.1, v_max * 1.3)], fixedrange=True, gridcolor='rgba(255,255,255,0.05)', tickfont=dict(size=9))
        
        # Mantengo tu posición actual del MW (No la toco)
        ...
        fig.update_layout(
            template="plotly_dark", height=270, # Reducimos altura
            margin=dict(l=30, r=5, t=10, b=30), # Margen superior (t) reducido al mínimo
            paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
            legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="center", x=0.5, font=dict(size=8), entrywidth=0.2, entrywidthmode="fraction"),
            hovermode="closest"
        )
