...
# --- CONFIGURACIÓN DE LOS 10 VOLCANES CHILENOS (IDs VALIDADOS 2026) ---
VOLCANES_CONFIG = {
...
def obtener_nivel_mirova(vrp, es_alerta):
    """Clasificación técnica basada en niveles de energía de MIROVA."""
    try:
        v = float(vrp)
...
def procesar():
    ...
    if not os.path.exists(RUTA_IMAGENES_BASE): os.makedirs(RUTA_IMAGENES_BASE)
    
    session = requests.Session()
    ahora_cl = obtener_hora_chile_actual()
    log_bitacora(f"🚀 INICIO CICLO V35.5 (RECLASIFICACIÓN): {ahora_cl}")
