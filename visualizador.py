...
# Rescate de Simbología y Colores del día 13
...
def procesar():
    ...
    for v in VOLCANES:
        ...
        if not df_v.empty:
            ...
            # Filtro de los últimos 30 días
            df_v = df_v[df_v['Fecha_Chile'] >= hace_30_dias]

        if not df_v.empty:
            # Gráfico con simbología recuperada
            ...
            # Look de "SIN ANOMALÍA" recuperado
            with open(ruta_v, "w") as f: 
                f.write("<body style='background:#161b22; display:flex; align-items:center; justify-content:center; height:100vh; margin:0;'><div style='color:#8b949e; font-family:sans-serif; font-size:16px; font-weight:bold; border: 1px dashed #30363d; padding: 20px; border-radius: 8px;'>SIN ANOMALÍA TÉRMICA (NOMINAL)</div></body>")
